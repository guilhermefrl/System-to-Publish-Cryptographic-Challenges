...
<div class="bg-white mt-0" id="teste"> <!--Menu Principal -->
  <ul class="nav nav-pills">
    ...
    <li class="nav-item">
      <a class="nav-link active" href="/SI/elgamal/descobrir.php">Resolver Desafio El Gamal</a>
    </li>
...
<div class="row">
  <div class="card mx-auto w-75 p-5">
      <h4><p class="text-info">Desafios El Gamal por resolver</p></h4>
<?php
      $sql = "SELECT * FROM desafio_elgamal;"; //Pesquisa todas as noticias que são definidas como visiveis
    ...
    while($row = mysqli_fetch_array($result2)){  //Guarda os valores na variavel row
     $id = $row['id_del'];
     $IDU = "SELECT * FROM elgamal_resolvido WHERE id_del = '"; 
     $IDU .= $id;
     ...
     $row1 = mysqli_fetch_array($result);
       if(empty($row1)){
          ...
          echo "
           ...
           <div class=\"card-body\">
            <h5 class=\"card-title\"><strong>P: ".$row['p']." G:  ".$row['g']."</strong></h5>
              <p class=\"text-secondary\">X:
             ".$row['x_mas']." </p>
             <p class=\"text-secondary\">Y:
             ".$row['y_mas']." </p>
             <p class=\"text-secondary\">k = Y^x mod P </p>
              </div>
              



             <form action=\"/SI/elgamal/calcular.php\" method=\"POST\">
              <div class=\"mr-2 ml-2\">
              <label><h5>Advinhe o valor da k para conseguir ler a mensagem</h5></label>                <!-- Campo para a mensagem -->
              <input type=\"number\" class=\"form-control\" placeholder=\"Advinhe K\" name=\"valorx\" required>
               </div>
             <div class=\"w-5 mt-2 ml-2 mb-2\">
              <input name=\"desafio\"type=\"hidden\"  value=$id> 
              <button type=\"submit\" class=\"btn btn-info\" name=\"decifrar_elgamal\">
                Resolver </button>
               </form>
...
<?php 
  if(isset($_GET['acertou'])){      //Mensagem de acerto
    echo '<script type="text/javascript"> window.onload = function(){ if(!alert("Parabéns! Acertou no valor de K")) document.location = "/SI/elgamal/descobrir.php";}</script>';
    echo '<script type="text/javascript"> window.open("/SI/elgamal/acertos/acertos.txt");</script>';
  } 
  if(isset($_GET['errou'])){      //Mensagem de erro
    echo '<script type="text/javascript"> window.onload = function(){ if(!alert("Este não era a resposta correta!")) document.location = "/SI/elgamal/descobrir.php";}</script>';
