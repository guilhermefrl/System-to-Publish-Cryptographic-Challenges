...
if($_SERVER["REQUEST_METHOD"] == "POST"){// Processa o formulário quando ele é enviado
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($email_err)){
                    // Verifica se os erros das variáveis estão todos vazios
 //Sql para inserir
        $sql = "INSERT INTO users (username,password,email,salt,tipo) VALUES (?,?,?,?,?)"; //Sql para inserir
        if($stmt = mysqli_prepare($link, $sql)){ //Prepara a execução dos parâmetros
            ...
            mysqli_stmt_bind_param($stmt,'ssssi', $param_username, $param_password, $param_email,$param_salt,$param_tipo);
            $iterations = 1000;
            // Set parameters
